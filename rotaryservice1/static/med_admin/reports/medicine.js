...
$(document).ready(function() {
    ...
    const dt = $table.DataTable({
        ...
        ordering: true,
        columnDefs: [
            { orderable: true, targets: [0,1,6,8] },
...
$("#export-btn").on("click", async function() {
    ...
    if (type === "pdf") {
        ...
        doc.text("Medicine Report", pageWidth / 2, topMargin + 25, { align: "center" });
    ...
    if (addedBy) filters.push({ label: "Issued By", value: addedBy });
    const rotorian = $("#rotorian").val();
    if (rotorian) filters.push({ label: "Rotarian", value: rotorian });
    ...
    const colWidths = [8, 25, 20, 10, 11, 11, 11, 21, 35, 13, 25];

    // --- Table ---
    doc.autoTable({
        startY: cursorY,
        head: [["SL", "Name", "Contact","Age", "Code", "PIN", "Token", "Added on", "Department", "Amount", "Medicine Confirmed"]],
        body: tableData,
        styles: {
            font: "NotoMalayalam",
            fontSize: 8,
        ...
        columnStyles: {
                ...
                10: { cellWidth: colWidths[10] }                    // Added By
            },
        theme: 'grid',
    ...
    // --- Save PDF ---
    doc.save(safeFileName('Medicine Report') + ".pdf");
}
...
if (type === "excel") {
    ...
    // ===== Headers =====
    const headers = ["SL", "Name", "Contact","Age", "Code", "PIN", "Token", "Added on", "Department", "Amount", "Medicine Confirmed"]; // ✅ added PIN
    ...
    link.download = safeFileName("Medicine Report") + ".xlsx";
    link.click();
}
