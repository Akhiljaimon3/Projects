...
$("#export-btn").on("click", async function() {
    ...
    if (type === "pdf") {
        ...
        doc.text("Consulted Report", pageWidth / 2, topMargin + 25, { align: "center" });
        ...
        doc.autoTable({
            startY: cursorY,
            head: [["SL","Name","Contact","Age","Code","Pin", "Token", "Added on","Department","Consultation Confirmed"]],
            body: tableData,
            styles: {
                font: "NotoMalayalam",
                fontSize: 8,
...
if (type === "excel") {
    ...
    const worksheet = workbook.addWorksheet("Consulted Report");
    let rowCursor = 1;
    ...
    // ===== Headers =====
    const headers = ["SL","Name","Contact","Age","Code","Pin", "Token", "Added on","Department","Consultation Confirmed"];
    ...
    link.download = safeFileName("Consulted Report") + ".xlsx";
    link.click();
}
